...
| `/dokumentumok` | GET | `PortalController::docs` | Dokumentumok - CMS intro + help cards (the document list binds to the public library in a later round) |
...
- **Removed / still TODO**: Forgalom (`/forgalom`) + Hibabejelentés (`/hibabejelentes`, tickets `cp_tickets`) were dropped from routes/views/nav; the topbar **notifications feed is empty** (no mock) until a real source; the Dokumentumok **document list is empty** (CMS intro + help cards only) until the public-library binding round (`App\Support\WebDocuments`, the rgtelekom pattern); the bank-transfer "Kifizetem" + invoice "Nyomtatás" stay visual.
